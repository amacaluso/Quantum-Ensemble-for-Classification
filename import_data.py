...
def load_bivariate_gaussian(n_train=20, plot=True, std=0.1):
    X, Y = datasets.make_blobs(n_samples=n_train, centers=[[0.3, 0.7], [0.7, 0.3]], # centers=[[0.3, 0.8], [0.7, 0.3]]
                               n_features=2, center_box=(0, 1), cluster_std=std,
                               random_state=543)
...
# load MNIST dataset
def load_MNIST(n=100, seed=123, train_size=.9):
    ...
    X_test = np.array([image_reshape(x, ncol, pca, MIN, MAX) for x in X_test])
